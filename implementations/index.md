...
* [drupal](https://www.drupal.org/project/drupal) The Drupal CMS supports exposing all data it manages (entities) according to the JSON:API specification with additional modules required as of 8.7.0. [`jsonapi_extras`](https://www.drupal.org/project/jsonapi_extras) is an optional extra module to change resource type names, field names and more. And [`openapi`](https://www.drupal.org/project/openapi) is another optional module, that is able to generate an OpenAPI/Swagger representation of the API provided by the `jsonapi` module with a ReDoc-powered UI. And [`jsonapi`](https://www.drupal.org/project/jsonapi) is a Drupal module that allows versions of Drupal 8 prior to 8.7.0 to expose data via JSON:API.
